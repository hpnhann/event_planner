...
// Event listeners để ẩn error message
const inputIds = ['loginEmail', 'password', 'forgotEmail', 'otpCode', 'newpassword', 'confirmpassword'];
inputIds.forEach(id => {
    ...
    if (element) {
        element.addEventListener('keyup', () => {
            ...
            error_msg.innerHTML = '';
            if (weakPswrd) weakPswrd.style.display = 'none';
            if (notMatch) notMatch.style.display = 'none';
        });
    }
});
...
function submitForm(event) {
    ...
    // Lấy return URL từ query string (nếu có)
    ...
    .then(data => {
        ...
        error_msg.innerHTML = data.status === "success" ? 'Đăng nhập thành công!' : data.message;
    
        errorbox.style.display = 'block';
    
        if (data.status === "success") {
            // SAU KHI LOGIN THÀNH CÔNG → LUÔN VỀ HOMEPAGE
            setTimeout(() => {
                if (data.return_url) {
                    // Nếu có return URL (từ event registration), về đó
                    window.location.href = data.return_url;
                } else {
                    // Mặc định về homepage cho tất cả roles
    ...
    .catch(error => {
        ...
        error_msg.innerHTML = '<strong>Lỗi</strong> ' + error.message;
        errorbox.style.display = 'block';
    });
}
...
document.getElementById('resendOTP').addEventListener('click', function() {
    ...
    .then(data => {
        document.getElementById('verifyCodeBtn').innerHTML = 'Verify Code';
        document.getElementById('resendOTP').innerHTML = 'OTP sent';
        setTimeout(() => {
...
function isStrongPassword() {
    ...
    if (!(password.length >= minLength)) {
        ...
        weakBadge.innerHTML = "Password must contain at least one uppercase letter.";
        weakBadge.style.display = 'block';
        return false;
    } else if (!(password.match(/([a-z])/))) {
        weakBadge.innerHTML = "Password must contain at least one lowercase letter.";
        weakBadge.style.display = 'block';
        return false;
    } else if (!(password.match(/([0-9])/))) {
        weakBadge.innerHTML = "Password must contain at least one number.";
        weakBadge.style.display = 'block';
        return false;
    } else if (!(password.match(/([!,%,&,@,#,$,^,*,?,_,~])/))) {
        weakBadge.innerHTML = "Password must contain at least one special character.";
        weakBadge.style.display = 'block';
        return false;
    } else {
        weakBadge.innerHTML = "";
